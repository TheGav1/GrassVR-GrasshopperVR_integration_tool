...
//
//=============================================================================
// Modify by TheGavi, for the implemantation of GaviVR Rhino connection Tool
//
//Difference:
//      -Level selection with level number (no name)[my mod]
//      -always additive load [my mod]
//      -Changed all SteamVR_LoadLevel to new class GaviVRLoadLevel
//      -Adding unload states and chenge level [my mod]
//=============================================================================
...
//Valve.VR namespace is kept as this scripti as an individual mod to be added
//to the genral Valve.VR codes. The following script will be called in the teleport 
//load level system and so remaining in the same name space will not required 
//adding "using Gavi.VR" or the calling of the name space in each execution 
//
namespace Valve.VR
{
    public class GaviVR_LoadLevel : MonoBehaviour
    {
        private static GaviVR_LoadLevel _active = null;
        ...
        public int notdestroylevelNumber = 0;
        ...
        public void Trigger()
        {
         //change   if (!loading && !string.IsNullOrEmpty(levelNumber))
            if (!loading && levelNumber != notdestroylevelNumber)
                StartCoroutine(LoadLevel());
        }

        // Helper function to quickly and simply load a level from script.
        public static void Begin(//change string levelName,
            int levelNumber,
            bool loadAdditive = false,
            bool loadAsync = false,//added to have asinc select in begin
            bool showGrid = false, float fadeOutTime = 0.5f,
            float r = 0.0f, float g = 0.0f, float b = 0.0f, float a = 1.0f)
        {
            var loader = new GameObject("loader").AddComponent<GaviVR_LoadLevel>();
         //change   loader.levelName = levelName;
            loader.levelNumber = levelNumber;
            loader.loadAdditive = loadAdditive;
        ...
        IEnumerator LoadLevel()
        {
            ...
            else
            {
                ...
                if (loadAsync)
                {
                    ...
                    //change async = UnityEngine.SceneManagement.SceneManager.LoadSceneAsync(levelName, mode);
                ...
                {
                    //change   UnityEngine.SceneManagement.SceneManager.LoadScene(levelName, mode);
